...
const AdminPanel = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (error) console.error("Login greška:", error.message);
    setLoading(false);
  };
  ...
  const handleDelete = async (id, naslov) => {
    if (window.confirm(`Obrisati: "${naslov}"?`)) {
      ...
      if (error) console.error("Greška pri brisanju:", error.message);
      fetchProjects();
    }
  };
  ...
  const handleFileUpload = async (event) => {
    ...
    } catch (error) { console.error("Upload greška:", error.message); } 
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#03040b] px-4">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/10 backdrop-blur-2xl p-10 rounded-[40px] border border-white/20 w-full max-w-md text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button className="w-full bg-cyan-500 py-4 rounded-2xl font-black text-black uppercase cursor-pointer">{loading ? "..." : "Pristupi"}</button>
          </form>
        </motion.div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen pt-40 pb-20 px-6 bg-[#03040b] text-white">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/5 backdrop-blur-3xl p-8 rounded-[40px] border border-white/10 mb-16 text-left">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="bg-cyan-500/5 border-2 border-dashed border-cyan-500/20 p-8 rounded-3xl text-center">
              <label className="block text-cyan-400 text-xs font-black uppercase mb-4">Izaberi materijal</label>
            ...
            <div className="p-5 bg-blue-500/5 border border-blue-500/20 rounded-2xl">
              <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-3 flex items-center gap-2"><FaInfoCircle /> Note</p>
              <div className="grid grid-cols-2 text-[9px] text-white/50 uppercase">
                <div>Slike: Max 2MB</div>
                <div>Video: Max 15MB</div>
              </div>
            </div>
            ...
            <button type="submit" className="w-full bg-cyan-500 p-5 rounded-2xl font-black text-black uppercase hover:bg-cyan-400 cursor-pointer">Objavi rad</button>
          </form>
        </div>

        <div className="space-y-4">
          {projekti.map(p => (
            <motion.div key={p.id} whileHover={{ x: 10 }} className="bg-white/5 p-6 rounded-3xl border border-white/5 flex justify-between items-center">
