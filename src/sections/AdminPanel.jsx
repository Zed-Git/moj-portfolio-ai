...
const AdminPanel = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Rešavamo "media_type unused" tako što direktno pravimo objekat za bazu
      const dataToInsert = {
  ...
  if (loading) return <div className="min-h-screen bg-[#020617] flex items-center justify-center text-cyan-500 font-mono animate-pulse uppercase">Syncing...</div>;
        ...
        >
          <div className="max-w-5xl mx-auto">
            <header className="flex justify-between items-center mb-10 bg-slate-900/50 p-6 rounded-3xl border border-white/5 shadow-2xl">
              ...
              <button onClick={() => supabase.auth.signOut()} className="bg-red-500/20 text-red-500 border border-red-500/20 px-6 py-2 rounded-xl text-[10px] font-black uppercase">
                End Session
              </button>
            </header>
            ...
            <div className={`bg-[#0f172a] border transition-all duration-500 p-6 md:p-10 rounded-4xl shadow-2xl mb-20 ${editingId ? 'border-yellow-500/50' : 'border-slate-800'}`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-start gap-3 bg-blue-500/10 border border-blue-500/20 p-5 rounded-xl text-blue-300 font-bold uppercase text-[11px]">
                  <FaInfoCircle className="mt-1" />
                  <p>NOTE: JPG, PNG and MP4 allowed. Max size: 50MB.</p>
                </div>

                <button type="submit" className={`w-full font-black py-5 rounded-2xl transition-all uppercase tracking-widest text-sm shadow-xl ${editingId ? 'bg-yellow-500 text-black' : 'bg-cyan-500 text-black'}`}>
              ...
              {projekti.map(proj => (
                <div key={proj.id} className="bg-[#0f172a] border border-slate-800 p-6 rounded-2xl flex justify-between items-center">
                  ...
                  <div className="flex gap-4">
                    <button onClick={() => startEdit(proj)} className="bg-slate-800 p-3 rounded-xl text-slate-400 hover:text-cyan-400"><FaEdit size={16} /></button>
                    <button onClick={() => handleDelete(proj.id)} className="bg-slate-800 p-3 rounded-xl text-slate-400 hover:text-red-500"><FaTrash size={16} /></button>
                  </div>
                </div>
              ))}
            </div>
