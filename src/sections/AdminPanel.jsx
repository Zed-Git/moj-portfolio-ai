...
const AdminPanel = () => {
  ...
  // Form State za NOVE PROJEKTE (Vraćamo tvoje stare funkcije)
  ...
  const handleAddProject = async (e) => {
    ...
    try {
      ...
      // Osveži listu
      const { data: updatedData } = await supabase.from('projects').select('*').order('id', { ascending: false });
      setProjekti(updatedData);
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6 text-white font-sans text-left">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-900 p-10 rounded-3xl border border-white/10 w-full max-w-md shadow-2xl">
          <h2 className="text-2xl font-black mb-8 uppercase tracking-widest italic text-center">ZED <span className="text-cyan-500 font-bold text-left">ADMIN</span></h2>
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full py-4 bg-cyan-600 hover:bg-cyan-500 text-white font-black rounded-xl transition-all uppercase tracking-widest">
              {loading ? <FaSpinner className="animate-spin mx-auto text-xl text-left" /> : 'Enter System'}
            </button>
          </form>
        </motion.div>
      </div>
    ...
    <div className="min-h-screen bg-black text-white p-6 md:p-12 text-left font-sans">
      <div className="max-w-6xl mx-auto">
        <header className="flex justify-between items-center mb-16 border-b border-white/10 pb-8 text-left">
          <h1 className="text-3xl font-black italic tracking-widest uppercase text-left">Admin <span className="text-cyan-500 italic text-left text-white">Panel</span></h1>
        ...
        <section className="mb-24 text-left">
          ...
          <AnimatePresence>
            {showAddForm && (
              ...
              >
                <input type="text" placeholder="Project Title" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none focus:border-cyan-500 text-left" value={naslov} onChange={(e) => setNaslov(e.target.value)} required />
                <textarea placeholder="Scientific Description" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none focus:border-cyan-500 h-32 text-left text-white" value={opis} onChange={(e) => setOpis(e.target.value)} required />
                <div className="flex items-center gap-4 p-4 border border-dashed border-white/20 rounded-xl text-left text-white">
            ...
            {projekti.map((proj) => (
              <div key={proj.id} className="bg-white/5 p-6 rounded-2xl border border-white/10 flex flex-col justify-between hover:border-cyan-500/30 transition-all min-h-[160px] text-left">
                <div className="text-left">
                  ...
                  <p className="text-[11px] text-slate-500 line-clamp-2 text-left font-light">{proj.opis}</p>
                </div>
                <div className="flex justify-end gap-4 mt-6 text-slate-500 border-t border-white/5 pt-4">
                  <FaEdit className="hover:text-cyan-400 cursor-pointer text-lg transition-colors text-left" />
                  <FaTrash onClick={async () => { if(window.confirm("Obriši?")) { await supabase.from('projects').delete().eq('id', proj.id); window.location.reload(); } }} className="hover:text-red-500 cursor-pointer text-lg transition-colors" />
                </div>
              </div>
            ))}
          </div>
